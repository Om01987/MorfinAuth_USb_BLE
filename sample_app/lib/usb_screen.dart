...
class _UsbScreenState extends State<UsbScreen> implements BottomDialogRefreshListener {
  ...
  Future<DeviceInfo?> GetDeviceInfo() async {
    ...
    DeviceInfo deviceInfoObject = CommonWidget.convertStringToDeviceInfo(deviceValue);
...
""";
  ...
  Future<void> GetSupportedDevices() async {
    ...
    if (ret == 0) {
      ...
      if (count > 0) {
        ...
        if (commaSeperated.isNotEmpty) {
          commaSeperated = commaSeperated.substring(0, commaSeperated.length - 2);
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('USB Dashboard'),
      ),
